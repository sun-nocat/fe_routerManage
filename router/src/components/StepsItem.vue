<template>
  <div>
    <Steps :current="num">
      <Step title="随机分配ip地址" content="根据用户选择的vlan来来随机生成未分配的ip地址"></Step>
      <Step title="主机信息记录" content="记录待分配主机的详细信息"></Step>
      <!-- <Step title="必要信息填写" content="这里是该步骤的描述信息"></Step> -->
      <Step title="信息确认" content="确认信息填写是否完善"></Step>
      <Step title="成功" content="主机记录成功"></Step>
    </Steps>
    <!-- 主要部分的显示 -->
    <!-- 选择vlan部分 -->
    <div class="vlan_div" v-show="vlan_show">
      <Form
        ref="formValidate1"
        :model="formValidate"
        :rules="ruleValidate"
        ...
        class="vlan-div-form"
      >
        <FormItem label="vlans">
          <RadioGroup v-model="value" class="vlan-choose">
        ...
        <FormItem class="vlan-next">
          <Button v-show="next" type="primary" @click="handleNext('formValidate1')">
            下一步
            <BackTop></BackTop>
          </Button>
    ...
    <div class="button_div" v-show="form_show">
      ...
      <template class="ff">
        ...
        >
          <Collapse active-key="1" v-model="collapseVal" class="mycollapse">
            <Panel key="1">
              基本信息
              <div slot="content">
                <!-- name -->
          ...
          <FormItem class="vlan-next2">
            <Button v-show="next" type="primary" @click="handleSubmit('formValidate')">
              下一步
              <BackTop></BackTop>
            </Button>
    ...
    <div v-show="list_show">
      ...
      <div style="text-align: center;">
        <Button style="margin : 0 auto;" v-show="sub" type="primary" @click="confirm">
          提交
          <BackTop></BackTop>
        </Button>
        <Button
          ...
          v-show="sub"
          type="primary"
          @click="handleChange('formValidate')"
        >
          更改
          <BackTop></BackTop>
        </Button>
...
export default {
  ...
  methods: {
    ...
    // 第一个 下一步
    handleNext(name) {
      this.$refs[name].validate(item => {
        if (item) {
          this.num = 1;
          this.vlan_show = false;
          this.form_show = true;
          console.log(item);
          console.log(this.formValidate.vlan);
          console.log(this.formValidate.ip);
        }
      });
    },
        ...
        .then(function(response) {
          {
            ...
            if (data.status == "0") {
              //不可以分配
              _this.formValidate.vlan = "";
              _this.formValidate.ip = "";
              _this.$Modal.confirm({
                title: "注意",
                content: data.msg,
                onOk: () => {},
